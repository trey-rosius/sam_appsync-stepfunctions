...
```bash
sam_step_functions$ sam build --use-container
```
...
```bash
sam_step_functions$ sam logs -n StockCheckerFunction --stack-name sam_step_functions --tail
```
...
```bash
sam_step_functions$ pip install -r tests/requirements.txt --user
# unit test
sam_step_functions$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam_step_functions$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam_step_functions
```
